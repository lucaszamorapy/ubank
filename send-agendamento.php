...
$servername = "postgres://default:FOMyb7WwN6RP@ep-round-wind-a4spkjhx-pooler.us-east-1.aws.neon.tech:5432/verceldb?sslmode=require"; // Endereço do servidor MySQL
$username = "default"; // Nome de usuário do MySQL
$password = "FOMyb7WwN6RP"; // Senha do MySQL
$database = "db_ubank"; // Nome do banco de dados
...
try {
  $stmt = $conn->prepare("INSERT INTO formulario (nome, email, message, date) VALUES (?, ?, ?, ?)");

  $stmt->bind_param('ssss', $name, $email, $message, $date);

  $stmt->execute();

  $gid = $conn->prepare("SELECT MAX(id) FROM formulario");
  $gid->execute();
...
try {
  ...
  $mail->setFrom('avellar.dev@gmail.com', 'Avellar Dev');

  $mail->AddAddress('avellar.dev@gmail.com', 'Avellar Dev');
  $mail->AddBCC('avellar.dev@gmail.com', 'Avellar Dev');
